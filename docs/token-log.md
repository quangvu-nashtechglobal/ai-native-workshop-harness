...
#c1 variant-b pre "FRESH session (exit + rerun copilot, or /clear)" !must; prompt "Read ONLY docs/product-brief.md and docs/knowledge/cyber-nexus-spec.md and agents/ba.agent.md — no other files — then draft user story US-02 'drag & drop task across columns' with testable acceptance criteria, following the story-shape in agents/ba.agent.md."; then "transcript shows 3 file-reads; keep story → docs/requirements/user-stories.md; commit".
...
#c4 a claim; what "gotcha-skills prevented known failure loops"; number "reference build paid 2 debug turns on dnd toast trap; this run paid 0 (skill pre-warned)"; evidence "skills/dnd-kit.skill.md trap-1".
